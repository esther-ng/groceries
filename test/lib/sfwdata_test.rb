...
class SfwDataTest < ActiveSupport::TestCase

  PRODUCT_KEYS = %w( conditions description img_url name offer_item reg_price sale_price store_id valid_from valid_til )
  ...
  test "parse_to_model returns an array of formatted raw product objects" do
    VCR.use_cassette("qfc") do
      results = SfwData.parse_to_model
      assert_kind_of Array, results
